...
function clearOutput(){
  ...
  document.getElementById("outputSurname").textContent = "";
  document.getElementById("outputDni").textContent = "";
  document.getElementById("outputState").textContent = "";
}
Dropzone.autoDiscover= false;
$("#compose-photo-exist").dropzone({
    maxFilesize: 2, // in MB
    paramName: "file",
    uploadMultiple: true,
    previewsContainer: "#file-uploader-exist",
    createImageThumbnails: true,
    maxThumbnailFilesize: 2, // in MB
    maxFiles: 1,
    addRemoveLinks : true,
    hiddenInputContainer: "#file-uploader-exist .addfile",
    dictResponseError: 'Server not responding',
    renderMethod: "prepend",
    dictDefaultMessage: "Add Photo",
    clickable : "#file-uploader-exist .addfile",
    ...
    init: function(){
      ...
      this.on("addedfile", function(file) {
        ...
        $('#startExistance').click(function(e){
            e.preventDefault();
            e.stopPropagation();
            dropzoneExistance.processQueue();
            disableButtons(true);
      ...
      this.on("success", function(file, response) {
        ...
        if (response.answer == config['RESPONSEALREADYEXISTS']){
          ...
          document.getElementById("outputState").textContent = response['state'];
        }else{
